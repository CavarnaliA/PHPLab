...
- Query Parameters:
  - `search=browser` — поисковое слово. Сервер понимает, что именно искать (“browser”)
  - `title=Special:Search` — какая служебная страница обрабатывает запрос. 
  `%3A` = это символ `:` → получается `Special:Search`. Чтобы открыть страницу поиска Википедии (а не статью)
  - `profile=advanced` — расширенный режим/профиль поиска. чтобы поиск работал в режиме “расширенный поиск”
  - `fulltext=1` — искать по полному тексту/запуск полноценного поиска. чтобы искать не только по названию, но и по тексту внутри страниц
  - `ns0=1` — фильтр по namespace 0 (основные статьи). Чтобы искать только среди обычных статей (не среди служебных страниц, обсуждений и т.п.)
